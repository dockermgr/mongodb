...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202303200832-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.com
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Monday, Mar 20, 2023 08:32 EDT
# @@File             :  install.sh
...
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/mongodb"
APPVERSION="$(__appversion "$REPO/raw/${GIT_REPO_BRANCH:-main}/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="mongodb"
export APPDIR="$HOME/.local/share/srv/docker/mongodb"
export DATADIR="$HOME/.local/share/srv/docker/mongodb/rootfs"
export INSTDIR="$HOME/.local/share/CasjaysDev/dockermgr/mongodb"
...
HUB_IMAGE_URL="casjaysdevdocker/mongodb"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# image tag [docker pull HUB_IMAGE_URL:tag]
HUB_IMAGE_TAG="latest"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the container name Default: [casjaysdevdocker/mongodb-$HUB_IMAGE_TAG]
CONTAINER_NAME=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: mongodb
CONTAINER_HOSTNAME=""
...
if [ -z "$CONTAINER_NAME" ]; then
  CONTAINER_NAME="$(__container_name || echo "casjaysdevdocker/mongodb-$HUB_IMAGE_TAG")"
fi
...
if [ -n "$MONGODB_USERNAME" ]; then
  CONTAINER_USER_NAME="$MONGODB_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${MONGODB_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
